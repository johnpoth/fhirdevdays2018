...
public class Application {
    ...
    static class FhirRoute extends RouteBuilder {
        ...
        public void configure() {
            restConfiguration().component("undertow").port("{{http.port}}").host("{{http.host}}");

            rest("getPatientCondition")
                .get("{MRN}")
                .to("direct:getFhirPatientCondition");

            from("direct:getFhirPatientCondition")
                 .setProperty("id", simple("${header.breadcrumbId}"))
                 ...
                 .multicast()
                   ...
                   .setHeader(FhirConstants.PROPERTY_PREFIX + "resourceClass", simple("Condition"))
